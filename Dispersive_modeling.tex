...
the Boussinesq equations by \citet{schaffer1993boussinesq}
are implemented with a hybrid of the finite volume
...
\citet{titov1995modeling,Imamura1996b,Harig2008,BergerGeorgeLeVequeMandli11}.
...
a function of propagation time and the shape of the disturbance \cite{Glimsdal2013},
...
\citet{schaffer1993boussinesq} introduced the concept of the {\em surface roller}. \citet{lynett2006nearshore} investigated $\eta_t/c$, $\eta_x$, $u_{s_{xx}} H^2/c$, and $u_s/c$, 
...
\marginpar{\footnotesize  Geir I don't understand figure 2a}
...
In Figure \ref{fig:boussclaw_grids}, 
...
\begin{figure}[!htb]
\centering
\includegraphics[width=.9\textwidth]{_fig/boussclaw_grids_t18.png}\\
\includegraphics[width=.9\textwidth]{_fig/boussclaw_grids_t20.png}\\
\includegraphics[width=.9\textwidth]{_fig/boussclaw_grids_t22.png}
\caption{Snapshot of the \textsc{BoussClaw} results 
at t=$18$, $20$ and $22$
with $\Delta x=0.025$, $0.05$ and $0.1$ m.}
\label{fig:boussclaw_grids}
\end{figure}
...
with those of other Boussinesq solvers, namely
 \textsc{Funwave} \citet{shi2012high}, \textsc{GloBouss} \citet{lovholt2010coupling} and the Serre type formulation \citet{Lovholt:2013a}.
As noted above, 
...
switched to the shallow water equations in the vicinity of the wave crest\marginpar{\footnotesize Geir: what is happening here ?}.
...
$A/h=1.97$, the free surface speed is $u_s/c=1.04$ and maximum surface slope angle of $39.1^\circ$.
\marginpar{\footnote Geir: how do we compute $u_s$ ? Is it the speed of tghe crest location}
\begin{figure}[!htb]
\centering
...
\begin{figure}[!htb]
    ...
    \end{subfigure}
    \caption{Energy plots of SWE and \textsc{BoussClaw}.
...
\label{append:stab}
It is difficult to analyse the numerical stability for  our full Boussinesq
equations. 
...
depth $h=1$. The equation replaces the momentum equation, wheras no separate
continuity equation is involved.

Following, as far as possible, the steps of section \ref{sec:Num_scheme}
we rearrange 
the equation (\ref{eq:basic_1}) as
\begin{align}
(I-D)(u_t + u_x) +Du_x = 0, \label{eq:basic_2}
\end{align}
where $D=\partial_x^2$.
...
accuracy on a regular grid and a uniform time step we may employ a standard von Neumann analysis where we calculate the growth of an harmonic mode over a single time step.
Expressing the coefficients of the velocity array before the time step as  $u_j= e^{i\xi j \Delta x}$ we then replace the coefficient of $\textbf{M}^q$, defined in section \ref{sec:Num_scheme}, by  $M_j^q=U_j^q= g_qe^{i\xi j \Delta x}$, 
where $q$ is 1, 2, 3, 4 or +. Correspondingly, the coefficients of the $\mathbf{S}^k$ array, which contains auxilary,  nodal values for $u_t$,  is
expressed $(S_j^k)= s^k e^{i\xi j \Delta x}$. 

We assume that the first step, where (\ref{eq:append_advec}) is solved, is absolutely 
stable. Hence, we simplify by assuming that $g_1$ is constant and has an absolute value
less than unity. 
...
\begin{align*}
& S_j^k - \frac{S_{j+1}^k-2S_j^k+S_{j-1}^k}{\Delta x^2} = 
-\frac{U_{j+2}^k - 2U_{j+1}^k +2U_{j-1}^k -U_{j-2}^k}{2\Delta x^3},
\end{align*}
which, inserted the harmonic expression, implies
\begin{align}
s^k=i\gamma g^k,\quad \gamma = \frac{ 2\sin(\xi \Delta x)(1-  \cos(\xi \Delta x)) }
                     { \Delta x^3 +2\Delta x(1-\cos(\xi \Delta x))}.
\label{append:rk4_1}
\end{align}
...
By combination of (\ref{eq:rk4_Neumann}) and (\ref{append:rk4_1}) $S^k$ and $g^k$, $k=1..4$ can be calculated sequentially  and combined in (\ref{append:rk4_assemble}) to provide
the value of $g^+$, 
\begin{align*}
g^+(\gamma) & = \left(1-
...
\end{align*}
If $|\gamma|<2\sqrt{2}$  then $|g^+(\gamma)/g^1|<1$. 
