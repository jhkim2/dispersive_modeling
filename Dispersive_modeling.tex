...
Sections \ref{sec:num_nonbreaking} compares results from the \BoussClaw with analytic ones, laboratory experiments and those from other models. 
...
Moreover, when presenting results we will often use a normalized time $t^*=\sqrt{g/h_0}t$ and position $x^*=x/h_0$, respectively, where $h_0$ is a reference depth.
...
such that $H$ is replaced by $d^*$ \todo{Geir: correct ?}
...
The amplitudes decreases very gently as the wave propagates.\todo{Change labels on axes to $\eta/h$ and $x/h$ andchange $t$ to $\hat t$ in figure 1. J:edited} 
...
\end{figure}
\todo{Change axis labels in figure 2 J:edited}
...
and generated solitary waves.
The gauge data from three cases are provided 
where the ratio $A/h_0$ equals  $0.038$, $0.259$ and $0.681$, respectively,
and the depth at the wavemaker is  $h_0=21.8\cm$. \todo{What is the wave shape? J: solitary waves. edited}
...
data of which can be found at \cite{synolakis2007standards}.
