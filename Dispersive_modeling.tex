...
Furthermore, the  wave steepening and breaking 
...
The  NLSW models do not incorporate frequency dispersion, which may be included by 
ascending in the hierarchy of long wave expansion 
...
However, most of the recently developed Boussinesq models are based on some combination of approximate Riemann solvers, with TVD limiters, for the hydrostatic transport terms and finite differences for dispersion terms  \citep{Erduran2005,Kim2009,Shiach:2009,Roeber:2010,Dutykh:2011,shi2012high}. Among other models, this has led to the popular
...
should be made for only a sub-region of the computational domain.
...
\BoussClaw model is somewhat more advanced toward  the beach than that from the BIM model.
...
we obtain $u^*/\sqrt{H^*} = 1.034$ and the surface slope angle of $39.1^\circ$ at the peak. 
%\todo{OK?}
For the present case this might indicate 
that the value of $u^*/\sqrt{H^*}$
%\marginpar{Is it the maximum or value at the peak? J:it's the value at the peak}
...
A $0.8$ threshold may well work better on a much gentler slope 
where the shape would remain close to that of a solitary and the wave  would start to 
spill when $A/h$ surpasses the maximum possible value for a solitary wave, which is close to
$0.8$.  
On the other hand, $1:19.85$ slope is already quite gentle,
...
from the Research Council of Norway under grant number
231252/F20 (Project TsunamiLand).
We also thank one anonymous referee for his helpful comments and suggestions.
All the data in this manuscript is accessible and can be obtained by email to jik@ngi.no
