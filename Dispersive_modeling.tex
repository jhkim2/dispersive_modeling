...
switched to the NLSW for the corresponding wave
% region, 
...
\end{equation}
where $C_d$ \! s/m$^{1/3}$ is a Manning drag coefficient, 
...
from including non-wetted nodes and thereby produce irregularities and even instabilities, the dispersion terms are switched off when $H<Nd_{tol}h_0$, where the number $N$ is set to 100 for the simulations herein.
...
\textrm{and}\quad u(x,t) = c \frac{\eta(x,t)}{h_0},
\end{flalign}
where
\begin{flalign}
\begin{split}
...
%\todo{Change labels on axes to $\eta/h$ and $x/h$ andchange $t$ to $t^*$ in figure 1.} 
...
\caption{Water surface elevation (m) in time (s) at gauges 5,7 and 8 for $A/h=0.259$ case.}
...
\todo{Make a proper reference of the URL J: edited} 
...
of $C_d = 0$ and $0.02$.
\todo{But, $C_d$ is not dimensionless....} 
...
\begin{figure}[tbh!]
	...
	\caption{Comparison of experiments and BoussClaw results 
		at t=25 and 30 with $\Delta x = 0.05$.}
...
\begin{figure}[!htb]
	\centering
		...
		\caption{Run-up height calculated from \BoussClaw and NLSW with different $C_d$.}
		\label{fig:runup_slope_1_20}
\end{figure}
\begin{table}
		\begin{tabular}{cccc} \hline
			 & \BoussClaw & NLSW & Experiment \\ \hline
			$C_d=0$ & 1.634 & 0.936 & 0.551\\
			0.01 & 0.921 & 0.702 & \\
			0.02 & 0.691 & 0.596 &\\ 
			0.03 & 0.576 & 0.528 &\\ \hline
		\end{tabular}
	\caption{Maximum run-up height with different friction coefficients.
		Maximum run-up measured by \cite{synolakis1987runup} experiment is 0.551.}%
...
So far, we have not used any threshold 
for the wave break.
...
we obtain $u/\sqrt{gH} = 1.034$ and a maximum surface slope angle of $39.1^\circ$. \todo{OK?}
...
very moderate reduction. Presumably, he increase is  due to the absence of strict energy  conservation in the Boussinesq equations as such. 
...
it is more difficult to assess the model with and without the switch to the NLSW.
...
yields too little dissipation. The last step of the procedure in section \ref{sec:Num_scheme}, which
...
the non-dissipative Serre model which may be run beyond $x_c=4$, without the strong artificial amplification of the Peregrine type models, but breaks down when the wave reaches the shoreline (results not shown). Hence, \BoussClaw, without the switch to a NLSW equation at
...
of breaking. 
However, herein we find also a very good pre-breaking performance of \BoussClaw Boussinesq 
equations where only some nonlinearity is retained in the dispersion. This suggest that
practice of retaining full nonlinearity in Boussinesq shoaling/runup models may be 
relaxed, especially when switch to NLSW equations are invoked when the wave-height to
depth ratio surpasses a threshold of $0.8$. This may help reducing stability problems 
that are observed for fully nonlinear Boussinesq equations \citep{Lovholt:2013}.
...
\begin{flalign}
& e_0 = \frac{\rho}{2}\left( g\eta^2 + H\bar{u}^2 \right), \label{eq:energy_e0}\\
