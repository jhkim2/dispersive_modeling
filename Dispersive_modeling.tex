...
Dimensionless time and position are then defined by, respectively,
\begin{equation}
\label{eq:norm_coord}
...
\ref{sec:comp_test}, \ref{append:stab} \ref{append:energy}, and finally the figures \ref{fig:bp5_water_tank}, \ref{fig:bp5b_gauges} and \ref{fig:init_setup}. Elsewhere, dimensionless variables are mainly employed.
...
\begin{flalign}
\label{eq:anal_serre}
 \eta^*(x^*,t^*) = \alpha\, \textrm{sech}^2 \left( \kappa (x^*-ct^*)\right),\quad
...
\begin{figure}[!htb]
    ...
    \end{subfigure}
    \caption{Evolution of a solitary wave for $\alpha=0.2$ and $\Delta x^*=0.1$.
    (a): Different parts of the dimensionless energy $E^*=E/(\rho g h_0^2)$.
    with $\Delta x^* = 0.1$. (b): 
...
described in \ref{append:energy}. These quantities are made dimensionless by $E_c=\rho g h_0^3$, which is minus twice equilibrium potential energy per width.
...
where the ratio $\alpha$ equals  $0.038$, $0.259$ and $0.681$, respectively,
...
\caption{Comparison of \BoussClaw and experiment from \cite{chl_bp5}. Water surface elevation (m) in time (s) at gauges 5,7 and 8 for $\alpha=0.259$ case.}
...
\caption{Definition sketch for shoaling and runup of solitary waves. The scales on the axes are equilibrium depth, $h_0$.}
...
The bathymetry of the wave tank is composed of a horizontal bottom, where the equilibrium depth is $h_0=$\marginpar{What is $h_0$ ?} and 
a uniform slope as shown in Figure \ref{fig:init_setup}. 
A solitary wave of height $A_0$, yielding $\alpha=A_0/h_0$, is generated at the right end of
...
As demonstrated in Figure \ref{fig:runup_meshrefinement}, 
...
\caption{Comparison of surfaces from the laboratory experiments \citep{synolakis1987runup} ($\alpha=0.28$, slope $1:19.85$), the BIM, the Serre, the \BoussClaw and the NLSW models at $t^*=15$.}
\label{fig:lab_bim}
\end{figure}
...
\begin{figure}[tbh!]
	...
	\caption{Comparison of experiments and BoussClaw results
                 for the breaking case ($\alpha=0.28$ and a $1:19.85$ slope). 
...
A detailed discussion of such effects are outside the scope of the present article, but 
a profound investigation is found in \cite{antuono2012role}.  
...
\begin{figure}[!htb]
	\centering
		...
		\caption{Run-up height for breaking case ($\alpha=0.28$ and a $1:19.85$ slope) computed by \BoussClaw 
...
\begin{table}
		\begin{tabular}{cccccc} \hline
			...
			\hline
		\end{tabular}
	\caption{Maximum run-up height for a solitary wave of amplitude $\alpha=0.28$ incident on  a $1:19.85$ slope.}%
...
\caption{Evolution of wave profiles for $\alpha=0.28$ and a $1:19.85$ slope. 
...
The energy then drops momentarily due to the change of energy formula, then remains constant until the wave breaks ($x^*_c$ around 6), after which a strong dissipation ensues.
...
\caption{Energy dissipation rates for $\alpha=0.28$ and a $1:19.85$ slope. The label ``formula'' represents (\ref{eq:energy_dissipation}) inserted the wave heights from the numerical simulations.
...
the non-dissipative Serre model which may be run beyond $x^*_c=4$, without the strong artificial amplification of the Peregrine type models, but breaks down when the wave reaches the shoreline (results not shown). Hence, \BoussClaw, without the switch to the NLSW equation at
...
The experiments of \citet{synolakis1987runup} and a full potential reference model
 enabled us to
assess a set of different long wave models, including \BoussClawt. Using the potential  model, we were able to assess in detail the pre-breaking behavior of the models,
...
first term, which is of order $\epsilon^2$ relative to equilibrium energy, is then associated with wave.
However, in the swash zone and during draw-down this distinction is not fruitful. Hence, instead of omitting the
...
wave that does not affect the shoreline. Then, the total energies per width are defined as 
\begin{flalign}
\label{eq:int_total}
 E_0=\int\limits_{x_a}^{x_b}e_0 dx-\int\limits_{x_0}^{x_b}-\frac 1 2 \rho g h^2 dx  ,\quad\quad  E_1=\int\limits_{x_a}^{x_b}e_1 dx,
\end{flalign}
where $x_a$ is the position of the instantaneous shoreline, $x_0$ is the position of the equilibrium shoreline, and $x_b$ denote off-shore boundary of the computational domain. The latter term is in $E_0$ yield subtraction of the initial equilibrium energy. For other geometries, these expressions must be modified accordingly.
