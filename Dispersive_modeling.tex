...
\citet{schaffer1993boussinesq}\todo{Is it Schaeffer or M and S ?} derived 
...
\end{flalign}
\todo{Is the Manning term included in $\Psi$ ?}
...
trough following the crest that surpases the treshold, \todo{G: May have to modify according to new simulations.},
...
from including non-wetted nodes and thereby produce irregularities and even instabililities, the dispersion terms are switched off when $H<Nd^*$, where the number $N$ is set to 100 for the simulations herein.\todo{G: Jihwan, check this}  
...
surface from the \BoussClaw are due\todo{Check statement on wrinkles} to the
