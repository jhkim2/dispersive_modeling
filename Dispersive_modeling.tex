...
In order to validate \BoussClaw, 
...
first proposed by \citet{Svendsen:}, \todo{missing reference}
...
However, there is a natural desire to exploit the effecient and well established shock capturing framework for NLSW models also in a dispersive context.
Hence, recently developed of operational Boussinesq models are often based on hybrid numerical techniques
such as use of approximate Riemann solvers combined with TVD limiters for the conserved variables, and finite differences
for additional higher order terms \citep{Erduran2005,Kim2009,shi2012high}. Among other models, this have led to the popular
...
In this paper, we present a new hybrid Boussinesq type model \BoussClaw, 
...
called \BoussClaw, is introduced. 
...
(\ref{eq:madsen_momx}) and (\ref{eq:madsen_new_op}) do not constitute a fully nonlinear set of Boussinesq, 
\todo{RJL: I'm not sure what you mean by ``do not constitute a fully nonlinear
set of Boussinesq''?} 
...
\label{sec_add_num}
Following \cite{Shi...201} we may catch wave breaking in a heuristic fashion 
...
Friction terms are necessary in the numerical simulations
to capture the inundation appropriately. 
For example, the Figure \ref{fig:BoussClaw_runup}
shows that the run-up height is larger than the experiments 
...
\textsc{Funwave-TVD} model shares important features with \BoussClaw, employing a hybrid 
...
snapshots from the \BoussClaw's simulation are shown 
...
\todo{RJL: Is it possible to increase the fontsize in labels of Figure 2b?}
...
Still, this may be a challenging task for Boussinesq type models\cite{Lovholt:2013a}. Runup on a $10^\circ$ was investigated experimentally
...
for $A/h=0.28$ and a $1:19.85$ slope at $t=15$. This is before the w3ave breaks and
both \BoussClaw and the BIM model are in good agreement with the experiments\marginpar{\footnotesize Also include the BoussClaw
results and NLSW in fig.~\ref{fig:lab_bim}}.
...
even seem to match the truncated swash tongue of the experiments well. However, this may a coincidence. Even though
the the wave has broken and some irregular flow features are introduced thereby, we have no evidence of the flow state being
...
Previously, \citet{wei1995fully} compared the Boussinesq equations
and potential flow model, and observed similar results. 

\subsection{Wave breaking and run-up}
For an incident amplitude of $A/h=0.28$ and a $1:19.85$ slope the \BoussClaw model,
the treshold $\epsilon_B=0.8$ (se sec.~\ref{sec_add_num}) is  reached at $t=14.9$
...
At $t=20$ the simulation with $\epsilon_B=0.8$ have already been in NLSW mode for $5$ time units and the difference in the wave height 
...
and the difference in the run-up height is relatively small. 
At t=30, two \BoussClaw results are similar
in the run-up height, but irregularities are observed
around $x=8$ for the \BoussClaw simulation with $\epsilon_B=0.8$. 
...
When the crest in the \BoussClaw simulation reaches $x=4.09$ we find
...
In the right panel, 
$E_0+E_1$ is well-preserved for the \BoussClaw simulation during deep water propagation, but
...
\todo{RJL: Here and in the abstract, is ``operational'' the right word? 
To me this means a code that is used in practical work such at 
tsunami warning centers.}
Boussinesq type model, \BoussClaw,
...
\BoussClaw resembles much used operational models such as \textsc{Funwave-TVD} and
...
enabled us to
constrain a set of different long wave models, including \BoussClaw, as well as a full potential
