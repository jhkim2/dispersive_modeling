...
first proposed by \citet{Svendsen:}, which is volume of water passively riding at
the bore front.
...
Previously, \cite{wei1995fully} compared the Boussinesq equations
...
At the coastline, the \textsc{BoussClaw}
use the \textsc{Geoclaw} software 
which can handle 
...
so that the water depth is set as $H=0$
...
In this work, the parameter $d^*$ is set as $10^{-4}$. 
