...
This allows for a relatively robust treatment of the modeling of the post breaking phase. To this end, \cite{tonelli2009hybrid} and \cite{shi2012high}, for instance, employ the $A/h = 0.8$ threshold which is motivated by the maximum 
...
Section \ref{sec:shoaling_runup} discusses the wave steepening and using a Boundary Integral Method (BIM) for 
solving the full potential equations. 
In Section \ref{sec:shoaling_breaking}, 
...
\citet{schaffer1995further} \todo{Is it Schaeffer or M and S ? J: same as S\&M(1995). Although M\&S(1992) is mathematically same, 1995 form seems to be more stable.} derived 
...
where
\begin{flalign}
\Psi(x,t)= & \left(B+\frac{1}{2} \right) h^2 \left( (Hu^2)_{x} + g H \eta_x \right)_{xx} -f_D\nonumber\\
& -\frac{1}{6}h^3 \left( \frac{ (Hu^2)_x +gH\eta_x }{h} \right)_{xx}
-Bgh^2\left(h\eta_x\right)_{xx}. &
\label{eq:madsen_new_disp_x}
\end{flalign}
\todo{Is the Manning term included in $\Psi$ ? J:It is included in $\Psi$, but computationally we solve $(hu)_t = -f_D$ separately}
...
switched to the NLSW for the corresponding leading wave, 
...
However, for the dry land inundation, the \BoussClaw invariably
...
Furthermore a parameter $d^*$ is chosen
to set $H=0$
...
from including non-wetted nodes and thereby produce irregularities and even instabilities, the dispersion terms are switched off when $H<Nd^*$, where the number $N$ is set to 100 for the simulations herein.
...
at $t=0,4,8$ and $12$ with $A/h=0.2$ and $\Delta x=0.05$. 
...
\subsubsection{Run-up on a steep slope}
\label{sec:10degrunup}
On a $10^\circ$ slope an incident solitary wave of amplitude $A/h=0.3$ will not break until the end of the draw-down phase \citep{Grilli:1997}.
...
Synolakis' experiment data can be found at 
...
%\todo{Geir: But, Li and Raichlen obtained a smaller runup height; Manning friction ? Check.}{\em Add figure with shorelines and table with maximum runup heights}
