...
When the solution of the NLSW has a shock, 
\cite{tissier2011serre} approximated 
the energy dissipation across the shock, $D_{th}$, as 
\begin{flalign}
	D_{th} = \frac{\rho g }{4}
	\left( \frac{g (2h + A)}{2h(h+A)} \right)^{1/2}
	A^3,
\label{eq:energy_dissipation}
\end{flalign}
where $A$ is the amplitude of shock and $h$ is 
the undisturbed water depth. 
Figure \ref{fig:energy_decay} shows 
the energy dissipation from the numerical simulations
and the formula (\ref{eq:energy_dissipation}).
As the wave approaches the slope, it is steepened and a shock is formed, 
and the difference between the simulation and the formula
becomes small. 
