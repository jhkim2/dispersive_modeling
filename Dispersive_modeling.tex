...
\end{equation}
where $h_0$ is a reference depth. Dimensional variables will be used in the sections \ref{sec:model},
\ref{sec:comp_test}, appendix \ref{append:energy}, figures
...
\begin{figure}[!htb]
    ...
    \end{subfigure}
    \caption{Evolution of a solitary wave for $A/h_0=0.2$ and $\Delta x^*=0.1$.
    (a):The energy of a solitary wave
    with $\Delta x^* = 0.1$. (b): 
...
\caption{Comparison of \BoussClaw and experiment from \cite{chl_bp5}. Water surface elevation (m) in time (s) at gauges 5,7 and 8 for $A/h_0=0.259$ case.}
...
\caption{Definition sketch for shoaling and runup of solitary waves.}
...
\begin{figure}[!htb]
	\begin{floatrow}
		...
		\caption{Non-breaking solitary wave ($A/h_0=0.3$ and $\theta=10^\circ$). Maximum run-up divided by amplitude (R/A) 
        ...
        }{\caption{Non-breaking solitary wave ($A/h_0=0.3$ and $\theta=10^\circ$).
...
\caption{Comparison of surfaces from the laboratory experiments \citep{synolakis1987runup} ($A/h=0.28$, slope $1:19.85$), the BIM, the Serre, the \BoussClaw and the NLSW models at $t=15$.}
\label{fig:lab_bim}
\end{figure}
...
They are made dimensionless by the factor $E_c=\rho g h_0^2$. 
...
Thereafter, energy is quickly dissipated. For the \BoussClaw simulations $E_0$
...
\begin{figure}[tbh!]
	...
	\end{subfigure}
	\caption{Wave energy ($A/h_0=0.28$, slope $1:19.85$) as function of crest position. The vertical line is at $x=8.03227$  where $\epsilon_B=0.8$. (a): $E_0$. (b): Solid line is $E_0+E_1$ of \BoussClaw without $\epsilon_B$. With $\epsilon_B=0.8$, $E_0$ is shown in dashed line after the threshold is reached.
 }
...
\caption{Energy dissipation rates for $A/h_0=0.28$ and a $1:19.85$ slope. The label ``formula'' represents (\ref{eq:energy_dissipation}) inserted the wave heights from the numerical simulations.
