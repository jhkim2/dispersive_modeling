...
\end{equation}
where $C_d^2$, with SI units  $\s^2/\m^{2/3}$, is a Manning drag coefficient, 
...
equations where only some nonlinearity is retained in the dispersion. 
This suggest that 
the practice of retaining full nonlinearity in Boussinesq shoaling/runup models 
may be relaxed, especially when the switch to NLSW equations are invoked 
when the wave-height to
depth ratio surpasses a threshold of $0.8$. This may help reducing stability problems 
that are observed for fully nonlinear Boussinesq equations \citep{Lovholt:2013a}. \todo{Need to rewrite this sentence?}
