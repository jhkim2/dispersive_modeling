...
\marginpar{\footnotesize Your reference to \citet{schaffer1993boussinesq} and \citet{schaffer1995further} is arbitrary. Suggest that you put in a clear distinction here, i.e. that you derive your eq.s on the basis of these two sets}. 
The \textsc{BoussClaw}
...
While we refer to \citep{shi2012high} for details, we briefly note that \textsc{Funwave-TVD} is based the fully nonlinear 
Boussinesq equations of \citep{Chen06}. The numerical spatial representation in \textsc{Funwave-TVD} is MUSCL TVD scheme 
...
which employed $400$ grid with no friction. \marginpar{\footnotesize Something missing. 400 points?}
...
the presentation of the results. \marginpar{\footnotesize Please make sure that we introduce non-dimensionality the first time we use it. Not sure if it is here.} 
...
%In Figure \ref{fig:bous_timeseries}, the numerical results from the \textsc{BoussiClaw} are given, the amplitude of the incoming wave continues to increase and the wave does not break. 
...
%\caption{Computational results from \textsc{BoussiClaw} 
...
the \textsc{BoussClaw} are compared
...
On the right figure, 
$E_0+E_1$ is well-preserved for the \textsc{BoussClaw},
