...
$A/h=1.97$, the maximum free surface speed is $u_s/c=1.04$ and maximum surface slope angle of $39.1^\circ$.
The ratio $u_s/c$ is derived by \citet{matsuyama2007study} as
\begin{flalign*}
\frac{u_s}{c} = \frac{\eta}{H} - \frac{h}{3H}\left[
H\frac{\partial^2 \eta}{\partial x^2} 
- 2 \left(\frac{\partial\eta}{\partial x}\right)^2
\right].
\end{flalign*}
...
depth $h=1$. The equation replaces the momentum equation, whereas no separate
...
stable in the sense that a small damping is involved. However, the method employed is not suited for a von Neumann stability analysis and we thus apply this technique to the second step only. \marginpar{\footnote What can be said on the actual stability properties of the flux limiter method?}
 Hence, we may put $g_1$ to unity, but it is preferable to retain it in the calculations. 
...
\marginpar{\footnotesize: Geir: It is not clear to me what is meant }
