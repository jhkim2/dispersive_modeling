...
However, there is a natural desire to exploit the effeciennt and well established shock capturing framework for NLSW models also in a dispersive context.
...
The \textsc{BoussClaw} simulations are compared
...
For the \textsc{BoussClaw}, the parameter $B=1/15$ is used, 
...
and the difference from the \textsc{BoussClaw} result 
...
At the coastline, \textsc{BoussClaw}
...
\textsc{BoussClaw} uses the Manning-type frictions as follows,
\begin{flalign*}
...
t=$20$, $25$, and $30$ from \textsc{BoussClaw} and NLSW
with firction forces $C_d=0.02$.
We compared the NLSW and \textsc{BoussClaw} with $\epsilon_B=0.8$
and without the threshold. 
At t=20, the difference in the wave height is clearly observed. 
With $\epsilon_B=0.8$, the governing equations have 
already switched to NLSW, and the bore is formed at the front. 
At t=25 and 30, the wave is running up the slope, 
and the difference in the run-up height is relatively small. 
At t=30, two \textsc{BoussClaw} results are similar
in the run-up height, but small waves are observed
around $x=8$ for the BoussClaw with $\epsilon_B=0.8$. 

\begin{figure}[tbh!]
\centering
\includegraphics[width=.8\textwidth]{_fig/bim_boussclaw_etaB8}
\caption{Comparison of \textsc{BoussClaw} and NLSW 
